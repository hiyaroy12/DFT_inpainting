#https://github.com/knazeri/edge-connect/blob/master/scripts/metrics.py
#python CEEC/metric_cal/metrics.py --data-path [path to validation set] --output-path [path to model output]
...
for fn in sorted(files):
    if os.path.isfile(path_pred+ '/' + basename(str(fn))):
...
#             )
        ...
        print(
            str(index) + ' images processed',
            "filename: %s" %name, "PSNR: %.4f" % round(compare_psnr(img_gt, img_pred, data_range=1), 4),
            "SSIM: %.4f" % round(compare_ssim(img_gt, img_pred, data_range=1, win_size=51), 4),
            "MAE: %.4f" % round(compare_mae(img_gt, img_pred), 4),
        )
