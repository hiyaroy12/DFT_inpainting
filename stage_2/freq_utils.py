...
#######################################################################
##############      Color regular random bbox util      ###############
#######################################################################
...
# # train_masks = read_file('/home3/hiya/workspace/inpainting_fft/PConv-Keras/data/masks/irregular_mask/train_mask_flist.txt')
# # random.seed(1234)
# # masks = read_file('/home3/hiya/workspace/inpainting_fft/PConv-Keras/data/masks/test_mask/test_mask_flist.txt')
# # idx = np.random.choice(len(masks), 10000, replace=False)
# # train_masks = [masks[id_] for id_ in list(idx)]
# # test_masks = [masks[id_] for id_ in range(len(idx)) if id_ not in list(idx)]
...
from PIL import Image
def get_color_fft_images_irregular(x_im, return_mask=False, train=True, perc=None): # b x 3 x h x w 
#     x_im = np.squeeze(x_im, axis=1)
    # x_fft has first three channel as magnitude next 3 channels as phase
    N = len(x_im)
    assert len(x_im[0].shape)==3, "Image should be color" #pytorch 3 x h x w
    ...
    if train:
        sample_mask = train_masks
    else:
        if perc>0.0 and perc<=0.1:
            sample_mask = read_file('./data/irregular_test_mask/irregular_test_mask_per_0010.txt')
        elif perc>0.1 and perc<=0.2:
            sample_mask = read_file('./data/irregular_test_mask/irregular_test_mask_per_1020.txt')
        elif perc>0.2 and perc<=0.3:
            sample_mask = read_file('./data/irregular_test_mask/irregular_test_mask_per_2030.txt')
        elif perc>0.3 and perc<=0.4:
            sample_mask = read_file('./data/irregular_test_mask/irregular_test_mask_per_3040.txt')
        elif perc>0.4 and perc<=0.5:
            sample_mask = read_file('./data/irregular_test_mask/irregular_test_mask_per_4050.txt')
        else:
            sample_mask = read_file('./data/irregular_test_mask/irregular_test_mask_per_5060.txt')
#     sample_mask = train_masks if train else test_masks
    for k, mask_file in enumerate(random.sample(sample_mask, N)):
...
def make_one_masked_image(x_im, mask, return_mask=True): # 1 x 3 x h x w 
    # x_fft has first three channel as magnitude next 3 channels as phase
    N = len(x_im)
    assert len(x_im[0].shape)==3, "Image should be color" #pytorch 3 x h x w
    h, w = x_im[0].shape[1:]
